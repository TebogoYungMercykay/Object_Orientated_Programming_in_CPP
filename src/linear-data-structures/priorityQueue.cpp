...
PriorityQueue<T>::PriorityQueue(const PriorityQueue<T> &other)
    : Queue<T>::Queue(other.PriorityQueue<T>::dataStructure->clone()) {
  ascend = other.ascend;
}
...
PriorityQueue<T> &PriorityQueue<T>::operator=(const PriorityQueue<T> &other) {
  ...
  PriorityQueue<T>::dataStructure =
      other.PriorityQueue<T>::dataStructure->clone();
  return *this;
}

template <class T> PriorityQueue<T>::~PriorityQueue() {}

template <class T> void PriorityQueue<T>::insert(T el) {
  int iIndex = 0;
  bool bAdded = false;
  if (ascend == true) {
...
template <class T> void PriorityQueue<T>::reverse() {
  if (ascend == true)
    ascend = false;
  else
    ascend = true;
