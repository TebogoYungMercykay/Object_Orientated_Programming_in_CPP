...
template <class T>
Queue<T>::Queue(LinearStructure<T> *c)
  ...
  while (!myDeleting->isEmpty()) {
    myData = 0;
    try {
      myData = myDeleting->remove(icount);
    } catch (const char *e) {
      icount = icount + 1;
      rear = rear + 1;
    }
...
Queue<T>::Queue(const Queue<T> &other)
    : OrderedContainer<T>::OrderedContainer(
          other.Queue<T>::dataStructure->clone()) {
  front = other.front;
  rear = other.rear;
}
...
template <class T> void Queue<T>::insert(T el) {
...
template <class T> void Queue<T>::reverse() {
  ...
  while (!Queue<T>::dataStructure->isEmpty() && rear > 0) {
    myTest = true;
    try {
      myData = Queue<T>::dataStructure->remove(rear - 1);
    } catch (string *e) {
      rear = rear - 1;
      myTest = false;
    }
