...
template <class T>
Stack<T>::Stack(LinearStructure<T> *c)
    : OrderedContainer<T>::OrderedContainer(c) {
...
Stack<T>::Stack(const Stack<T> &other)
    : OrderedContainer<T>::OrderedContainer(
          other.Stack<T>::dataStructure->clone()) {
...
template <class T> void Stack<T>::insert(T el) {
...
template <class T> void Stack<T>::reverse() {
  ...
  while (!Stack<T>::dataStructure->isEmpty() && stackTop > 0) {
    ...
    myTempHolder->insert(myStackCount - stackTop,
                         Stack<T>::dataStructure->remove(stackTop - 1));
    stackTop = stackTop - 1;
  }
