...
OrderedContainer<T>::OrderedContainer(LinearStructure<T> *c) {
  ...
  while(!myDeleting->isEmpty())
  {
          ...
          myData=0;
          try { myData=myDeleting->remove(myIndex); } catch (const char * e) {
  myIndex=myIndex+1; } if(myData!=0)
          {
                  dataStructure->insert(myNewIndex,myData);
                  myNewIndex=myNewIndex+1;
                          //myIndex=myIndex+1;
                  cout<<"my current growing datastructure is
  "<<*dataStructure<<endl;
          }
...
template <class T>
OrderedContainer<T> &
OrderedContainer<T>::operator=(const OrderedContainer<T> &other) {
  delete dataStructure;
  dataStructure = other.dataStructure->clone();
  return *this;
}

template <class T> OrderedContainer<T>::~OrderedContainer() {
  dataStructure = NULL;
}

template <class T> bool OrderedContainer<T>::isEmpty() {
