...
Matrix Matrix::operator|(const Matrix &rhs_matrix) {
  ...
  if (cols != rows) {
    ...
    bool check = true;
    Matrix tempMatrix(*this), tempMatrix_2(rhs_matrix), mat_t(*this),
        tempMatrix_22(rhs_matrix);
    check = true;
    mat_t |= tempMatrix_22;
    int r = 0;
    for (; r < tempRows2;) {
    ...
    for (; r >= 0;) {
      int c = r + 1;
      for (; c < tempRows2;) {
        tempMatrix_2.matrix[r][0] =
            tempMatrix_2.matrix[r][0] -
            tempMatrix.matrix[r][c] * tempMatrix_2.matrix[c][0];
        c++;
      }
      if (tempMatrix.matrix[r][r] != 0) {
        tempMatrix_2.matrix[r][0] =
            tempMatrix_2.matrix[r][0] / tempMatrix.matrix[r][r];
      } else {
        throw "Error: division by zero";
      }
      r--;
