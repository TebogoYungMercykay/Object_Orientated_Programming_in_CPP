...
Calendar::Calendar() { this->head = NULL; }
...
Calendar::~Calendar() { this->clearCalendar(); }
...
Event *Calendar::getUpcomingEvent(long int now) {
  if (this->head == NULL) {
    ...
    while (eventPtr) {
      if (unix_to_string(eventPtr->start) > unix_to_string(now) ||
          eventPtr->start > now) {
        return eventPtr;
      }
...
vector<Event> Calendar::filterEvents(int *id2, long int *start2, long int *end2,
                                     string *type2) {
  vector<Event> filterEventsVector;
  Event *eventPtr = this->head;
  if (id2 == NULL && start2 == NULL && end2 == NULL && type2 == NULL) {
    while (eventPtr) {
      Event eventDataVec(eventPtr->start, eventPtr->end, eventPtr->id,
                         eventPtr->type);
    ...
    if ((id2 != NULL && type2 != NULL)) {
      while (eventPtr) {
        if (eventPtr->id == *id2 && eventPtr->type == *type2 &&
            eventPtr->end < *end2 && eventPtr->start > *start2) {
          Event eventDataVec(eventPtr->start, eventPtr->end, eventPtr->id,
                             eventPtr->type);
          filterEventsVector.push_back(eventDataVec);
        }
...
Event *Calendar::getFirstEvent() { return this->head; }
