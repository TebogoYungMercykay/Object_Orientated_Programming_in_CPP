...
void printDogs(Dog *array[]) {
  ...
  for (int k_iterator = 0; k_iterator < 10; k_iterator++) {
    if (array[k_iterator] != NULL) {
      printOut += array[k_iterator]->getName() +
                  "(Owner: " + array[k_iterator]->getCitizen()->getName() +
                  "), ";
    }
  }
  ...
  do {
    ...
    if (optionsVar != 3) {
      if (optionsVar == 1) {
        ...
        cin >> secondDog;
        if (containsDog(myDogArray, firstDog) != -1 &&
            containsDog(myDogArray, secondDog) != -1) {
          swapNames(*myDogArray[containsDog(myDogArray, firstDog)],
                    *myDogArray[containsDog(myDogArray, secondDog)]);
          printDogs(myDogArray);
        } else {
