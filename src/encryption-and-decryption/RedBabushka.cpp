...
RedBabushka::RedBabushka(const unsigned char *id, unsigned int rotationAmount)
    : Babushka(id) {
...
void RedBabushka::encrypt(unsigned char *data, int size) {
  ...
  if (this->rotationAmount >= size)
    ...
    for (int i = size; i >= this->rotationAmount; i--)
      data[i] = data[i - this->rotationAmount];
    for (int i = 0; i < this->rotationAmount; i++)
      data[i] = last[i];
  }
}
void RedBabushka::decrypt(unsigned char *data, int size) {
  ...
  if (this->rotationAmount >= size)
    ...
    for (int i = 0; i < this->rotationAmount; i++)
      first[i] = data[i];
    for (int i = 0; i < size - this->rotationAmount; i++)
      data[i] = data[i + this->rotationAmount];
    int x = 0;
    for (int i = size - this->rotationAmount; i < size; i++) {
      data[i] = first[x];
