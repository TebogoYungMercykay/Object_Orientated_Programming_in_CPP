...
GreenBabushka::GreenBabushka(const unsigned char *id, unsigned int displacement)
    : Babushka(id) {
...
void GreenBabushka::encrypt(unsigned char *data, int size) {
  ...
  unsigned char ftttt;
  if (this->displacement > size)
    throw DisplacementEx;
  else {
    std::string holder_thingie = "";
    for (int k = 0; k < size; k++)
      holder_thingie += data[k];
