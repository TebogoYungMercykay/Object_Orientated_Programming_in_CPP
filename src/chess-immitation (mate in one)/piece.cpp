...
// destructor now(>_<)...
piece::~piece() {
  std::cout << "(" << this->xPos << "," << this->yPos << ") " << this->side
            << " " << this->pieceType << " deleted"
            << "\n";
}
std::string piece::getPieceType() { return this->pieceType; }
int piece::getX() { return this->xPos; }
int piece::getY() { return this->yPos; }
...
std::ostream &operator<<(std::ostream &output, const piece &pieces) {
  output << pieces.side << " " << pieces.pieceType << " at "
         << "[" << pieces.xPos << "," << pieces.yPos << "]"
         << "\n";
  return output;
}
