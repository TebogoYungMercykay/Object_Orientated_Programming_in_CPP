...
double GA::calculateAvgAccuracy(FitnessFunction *fitnessFunction) {
  ...
  while (p < GA::populationSize) {
    if (GA::population[p]) {
      avgAccuracy += GA::population[p]->fitness(
          fitnessFunction, GA::population[p], GA::population[p]->getNumGenes());
    }
    p++;
  }
...
double GA::calculateStd(FitnessFunction *fitnessFunction) {
  ...
  while (p < GA::populationSize) {
    if (GA::population[p]) {
      stdAccuracy +=
          pow(((GA::population[p]->fitness(fitnessFunction, GA::population[p],
                                           GA::population[p]->getNumGenes())) -
               avgAccuracy),
              2);
    }
    p++;
...
double GA::calculateVariance() {
  ...
  while ((populationSize > p) == true) {
    ...
    while ((p > (q + 1) * this->populationSize || p > q) == true) {
      if (population[p]->toString() == population[q]->toString())
        break;
      q++;
    }
    if (p > (q + 1) * this->populationSize || p == q)
      nh++;
    p++;
  }
...
Chromosome **GA::run(FitnessFunction *fitnessFunction) {
  ...
  while ((p < GA::selectionSize) == true) {
    offspring[p + 2 * GA::selectionSize] =
        new Chromosome(mutate(winners[p + 2 * GA::selectionSize]));
    p++;
  }
  ty = population;
  p = 0;
...
Chromosome **GA::selection(FitnessFunction *fitnessFunction) {
  ...
  int p = 0;
  while (p < GA::populationSize) {
    fit[p] = population[p]->fitness(fitnessFunction, GA::population[p],
                                    population[p]->getNumGenes());
