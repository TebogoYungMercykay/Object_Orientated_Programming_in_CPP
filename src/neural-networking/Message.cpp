...
Message::Message(Message *msg) { this->data = msg->getData(); }
Message::~Message() {
...
bool IsIntergerNumber(std::string myS) {
  for (int k_iterator = 0; k_iterator < myS.length(); k_iterator++) {
    ...
    for (int p_iterator = 0; p_iterator < 26; p_iterator++) {
      if (myS[k_iterator] == myArr[p_iterator] ||
          myS[k_iterator] == myArr2[p_iterator]) {
        return false;
      }
    }
  }
...
std::string Message::getDestination() {
  if (this->Destination != "" && this->Destination != "\0") {
    ...
    while (k_iterator < 3 && getline(St, store_iterator, '.')) {
      if (IsIntergerNumber(store_iterator)) {
...
void Message::setEncoding(std::string Econding) { this->Encoding = Econding; }
bool Message::setOriginIP(std::string OriginIP) {
  if (this->getOriginIP() == "ERROR: IPV4 NOT SET") {
    if (OriginIP != "") {
      ...
      St >> myArray[3];
      int A = -1, B = -1, C = -1, D = -1;
      if (IsIntergerNumber(myArray[0]) && IsIntergerNumber(myArray[1]) &&
          IsIntergerNumber(myArray[2]) && IsIntergerNumber(myArray[3])) {
      ...
      // }
      // if(counter == 4 && (myArray[0]!="" && myArray[1]!="" &&
      // myArray[2]!=""
      // && myArray[3]!="")){
...
bool Message::setOriginPort(std::string OriginPort) {
  if (this->getOriginPort() == "ERROR: PORT NOT SET" &&
      IsIntergerNumber(OriginPort)) {
    int k_const;
...
bool Message::setDestinationPort(std::string DestinatioPort) {
  if (this->getDestinationPort() == "ERROR: PORT NOT SET" &&
      IsIntergerNumber(DestinatioPort)) {
    int k_const;
...
int Message::getContentLength() { return this->contentLength; }
