...
template <class T> double Plane<T>::calculateValue() {
  if (Plane<T>::cargo.empty() == !false) {
    ...
    for (int i = 0; i < size; i++) {
      if (Plane<T>::cargo[i]->getValue() >
              Plane<T>::cargo[leastIndex]->getValue() &&
          Plane<T>::cargo[i]->getValue() <
              Plane<T>::cargo[secondLeastIndex]->getValue()) {
        secondLeastIndex = i;
      }
    }
    if (Plane<T>::cargo[leastIndex]->getValue() <
        (Plane<T>::cargo[secondLeastIndex]->getValue()) / 2) {
