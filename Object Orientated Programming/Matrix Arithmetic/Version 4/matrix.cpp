...
Matrix operator*(const double& m, const Matrix& rhs)
{
    ...
    for(int i = 0; i < rhs.rows; i++)
    {
        ...
        {
            // multiply the elements of Matrix rhs and m and store them in a temp matrix
...
Matrix::Matrix(unsigned r, unsigned c)
{
    ...
    for(int i = 0; i < rows; i++)
    {
        ...
        for(int j = 0; j < cols; j++)
        {
            matrix[i][j] = 0;
            // OR *(*(matrix + i) + j) =0;
            // The matrix[i][j] is not a pointer, This will not work -> *matrix[i][j] = 0;
        }
...
Matrix::Matrix(const Matrix& rhs)
{
    ...
    for(int i = 0; i < rows; i++)
    {
        ...
        for(int j = 0; j < cols; j++)
        {
            ...
            //     //IS THIS CORRECT?
            //     // Yes: It's the perfect way of assigning the corresponding values.
            // }
            matrix[i][j] = rhs.matrix[i][j];
        }
    }
...
{
    // You do not need the if statement since they pass by reference, Meaning infile cannot be null.
    // infile has to be an existing object. And if it's empty nothing will be stored with or without the if statement eitherway.
    if(infile)
    {
        for(int i = 0; i < rows; i++)
        {
            for(int j = 0; j < cols; j++)
            {
                ...
                //IS THIS CORRECT?
                // Yes it is:
                // You can also use something like getline(infile, stringVariable, ' '); Then convert the string to double using stringstream and store the value afterwads.
                // OR store the val from infile and store them in a temp variable the set matrix[i][j] to be equal to the variable.
            }
...
Matrix Matrix :: operator+(const Matrix& rhs)
{
    ...
    if(rows==rhs.rows && cols==rhs.cols)
    {
        for(int i = 0; i < rows; i++)
        {
            for(int j = 0; j < cols; j++)
            {
                //Add the corresponding elements of Matrix this and Matrix rhs and store them in a temp matrix
...
Matrix Matrix :: operator-(const Matrix& rhs)
{
    ...
    if(rows==rhs.rows && cols==rhs.cols)
    {
        for(int i = 0; i < rows; i++)
        {
            for(int j = 0; j < cols; j++)
            {
                //Subtract the corresponding elements of Matrix this and Matrix rhs and store them in a temp matrix
                temp.matrix[i][j] = this->matrix[i][j] - rhs.matrix[i][j];
            }
...
Matrix Matrix :: operator*(const double& rhs)
{
    ...
    for(int i = 0; i < rows; i++)
    {
        for(int j = 0; j < cols; j++)
        {
            temp.matrix[i][j] = rhs*matrix[i][j];   //which is the same as temp.matrix[i][j] = matrix[i][j] * rhs;
        }  
    }
  return temp;  
}
