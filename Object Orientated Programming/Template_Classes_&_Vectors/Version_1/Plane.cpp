...
void Plane<T>::remove(T* var){
    ...
    const int vecSize = this->cargo.size();
    int index = -1;
    if(this->cargo.empty() != false){ throw EmptyEx(); }
    for(int i=0; i< vecSize; i++){
        if(this->cargo[i]!=var){
            continue;
        }
...
double Plane<T>::calculateValue(){
    if(this->cargo.empty() != false){
        ...
        for(int i = 0; i < vecSize; i++){
            bool var1 = (this->cargo[index2]->getValue()<this->cargo[i]->getValue() && this->cargo[index3]->getValue()>this->cargo[i]->getValue());
            if(var1){
                index3 = i;
            }
        }
        ...
        if(var2 > this->cargo[index2]->getValue()){
            throw UndervaluedEx<T>(this->cargo[index2], total-this->cargo[index2]->getValue());
        }
        return total;
    }
    return 0;
